...
# R6 (FEN-353, validated live on the NAS): the runtime base MUST ship glibc
# >= 2.39 — the official convex-local-backend binary we COPY in requires
# GLIBC_2.38/2.39, and node:24-bookworm-slim (Debian 12) only has 2.36, so the
# backend fails with "version `GLIBC_2.38' not found" and never boots. Debian 13
# (trixie) ships glibc 2.41. Keep build + runtime on the same base.
...
# R6 (FEN-353, validated live): `convex deploy` runs at runtime and bundles the
# functions in ./convex, which import shared code via `../app/lib/shared/*`
# (convex/routing.ts, routes.ts, seed.ts). That source must be present in the
# image or deploy fails with `Could not resolve "../app/lib/shared/geo"`. The
# app server runs from .output, so only the shared lib is needed here.
