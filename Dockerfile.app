...
# curl: container healthcheck. No build toolchain needed at runtime — we copy the
# already-installed (pruned) node_modules from the build stage.
RUN apt-get update \
...
# Copy the pruned prod deps (+ package.json for resolution).
