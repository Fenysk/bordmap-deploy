 ...
 *   origins into the env (VITE_CONVEX_URL build arg, SITE_URL, AUTH_ISSUER) →
 ...
 *  known. BETTER_AUTH_SECRET is generated + persisted if blank (a rotated secret
 *  would log every user out on the next deploy). */
function buildStackEnv(c) {
  ...
  const stack = {
    ...
    // Better Auth baseURL (public, for cookies + redirects).
    SITE_URL: base,
    // JWT issuer + Convex OIDC validation URL. Must be the INTERNAL Docker service
    // URL so convex-backend can reach it within the compose network. The external
    // HTTPS URL (bordmap.fenysk.fr) is unreachable from inside the container
    // (hairpin NAT / network isolation → ECONNREFUSED on JWKS fetch). The JWT `iss`
    // claim will be this internal URL, which is fine — Convex validates it
    // server-side against the same configured domain.
    AUTH_ISSUER: "http://app:3000/api/auth",
    BETTER_AUTH_SECRET: need("BETTER_AUTH_SECRET", () => genHex(32)),
...
async function main() {
  ...
  // Read the Coolify-assigned domain so the VITE_* build arg + SITE_URL are real.
  if (!c.publicBaseUrl) {
    try {
