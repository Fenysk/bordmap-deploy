...
## Amendment 2 — 2026-06-19 (FEN-740): graph build moved to a one-time runtime import on a persistent volume

**Decision:** supersede Amendment 1's **graph cache BAKED AT IMAGE BUILD** with a
**one-time runtime import onto a PERSISTENT volume** (`graphhopper-data:/data`). This
is the refined form of Amendment 1's retained fallback (*Option C — co-located runtime
import*), made safe by making the import happen **once per volume**, not per restart.

### Why the build-bake was abandoned

The FEN-602 build bake never delivered: it **silently produced an empty `graph-cache`**
(OOM / Geofabrik egress failure on the Coolify builder) — the `import` RUN was wrapped
to always succeed so the image built, then served an empty graph. FEN-603 added a
runtime-import self-heal to recover, but it imported into an **image-layer path**
(`/graphhopper/graph-cache`), which the container **recreate** on every redeploy throws
away → the ~10 min import recurred on every deploy. Diagnosing the empty bake required a
prod VPS shell that is not available ([FEN-739](/FEN/issues/FEN-739),
[[bordmap-prod-host-and-observability]]) — so rather than fix the unobservable bake, we
remove it.

### The new flow (reproducible, no prod shell needed)

1. `docker-compose.coolify.yml` mounts the persistent **`graphhopper-data`** volume at
   `/data` on the `graphhopper` service.
2. `config.yml` points `graph.location` → `/data/graph-cache` and
   `graph.elevation.cache_dir` → `/data/srtm` (on the volume, not an image path).
3. `import-and-serve.sh` imports **once** when `/data/graph-cache` is empty (fresh
   volume), then `exec … server`. Every subsequent boot finds the persisted graph and
   serves immediately — **a redeploy no longer re-imports**.
4. The `Dockerfile` no longer bakes (JAR + config only) → fast, deterministic builds
   with no builder-OOM failure mode. Region + import heap are **runtime env**
   (`GH_OSM_URL` / `GH_IMPORT_OPTS`).

**Blast radius:** the import runs in the runtime container (normal egress + RAM), capped
heap + MMAP. The one-time import costs a single ~10 min GH window on the **first** deploy
after this change (fresh volume); the healthcheck `start_period` is widened to 900s to
cover it. If that one import fails the entrypoint `sleep`s (no crash-loop). Base
Rhône-Alpes, no LM — the proven-green workload (FEN-601). Verification is the same
two-step redeploy from [[bordmap-coolify-deploy-state]] + the existing `ascend`
healthcheck/smoke.
