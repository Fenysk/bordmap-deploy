...
#   GRAPHHOPPER_URL               internal URL for routing engine
set -u
...
# which captures stdout — so any diag on stdout would (a) be swallowed from the
# served log and (b) POLLUTE the captured admin key. stderr keeps stdout = key-only,
# and the outer `2>&1 | tee` still routes these lines into the /diag bootstrap log.
...
deploy_convex() {
  ...
  # Diagnostics (FEN-96: Coolify hides container logs, so surface the bootstrap
  # over HTTP via BOOT_LOG). NEVER print the admin key value — only its length.
  keylen=0
  [ -n "${CONVEX_SELF_HOSTED_ADMIN_KEY:-}" ] && keylen=$(printf '%s' "${CONVEX_SELF_HOSTED_ADMIN_KEY}" | wc -c | tr -d ' ')
  log "diag: admin_key_len=${keylen} backend=${BACKEND_URL}"
  ...
  # The computeRoutes action (FEN-504) reads it server-side via process.env.
...
# stderr stream. (The /diag HTTP-served bootstrap log was a FEN-601 diagnostic,
# removed now that the routing deploy path is proven — computeRoutes is green.)
