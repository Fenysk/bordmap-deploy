...
if [ -n "${CONVEX_SELF_HOSTED_ADMIN_KEY:-}" ]; then
  # Self-hosted Convex keeps function env IN the deployment (not process.env), so
  # convex/auth.config.ts reads AUTH_ISSUER from the Convex env store. Behind a
  # public reverse proxy the Better Auth JWT `iss` = ${SITE_URL}/api/auth, so the
  # Convex provider `domain` must match or every authed query 401s. Push it before
  # deploy (best-effort — a failure degrades to anonymous reads, never blocks boot).
  if [ -n "${AUTH_ISSUER:-}" ]; then
    log "setting Convex env AUTH_ISSUER=${AUTH_ISSUER}"
    npx convex env set AUTH_ISSUER "${AUTH_ISSUER}" >/proc/1/fd/1 2>/proc/1/fd/2 || log "WARN: convex env set AUTH_ISSUER failed (continuing)"
  fi
